...
export default function UpdateForm({ setOpen, item }: UpdateFormProps) {
  const { id, name, image_path } = item;

  const form = useForm<CategoryForm>({
    resolver: zodResolver(CategoryFormSchema),
    defaultValues: { name: name, image_path: image_path, image: null },
  });
  ...
  const { mutate, isPending } = useMutation({
    ...
    onMutate: async ({ id, formData }) => {
      ...
      const { name, image, image_path } = formData;
      const updatedItem: Category & { isOptimistic: boolean } = {
        id: id,
        name: name,
