            ...
            <Card key={row.original.id} className={`text-sm p-2 gap-2 ${row.getIsSelected() && 'bg-muted'}`}>
              <div className="flex flex-col gap-2">
                ...
                <div className="text-gray-600 flex flex-col gap-0.5">
                  ...
                  <div>Venta: {formatCurrency(row.original.mto_precio_unitario)}</div>
                </div>
              </div>
                  ...
                  <div className="w-20 h-20">
                    {row.original.image_path ? (
                      <img
                        ...
                        src={`${import.meta.env.VITE_API_URL}/storage/${row.original.image_path}`}
                        onError={(e) => {
                          const img = e.target;
                          if (img instanceof HTMLImageElement) {
                            img.onerror = null;
                            img.src = row.original.image_path ? row.original.image_path : '/placeholder.svg';
                          }
                        }}
                      />
                    ) : (
